...
async function main() {
  ...
  const input =
    "What contaminated this site? What steps have been taken to clean it up? What is the current status?";
  ...
  const { text } = await generateText({
    model: openai("gpt-4o"),
    prompt: `You are a helpful assistant who is knowledgeable about contaminated waste Superfund sites in the US. You will be provided context written by scientists, and your job is to summarize in a manner readable to regular educated people. Answer the following question based only on the provided context:
             ${context}
...
export async function GET() {
  const text = await main().catch(console.error);
